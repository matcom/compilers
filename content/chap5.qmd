...
def get_ast(self) -> Expression:
    # X -> Y o Z
    if len(self.children) == 3 and self.children[1].symbol >= 2:
        return BinaryExpr(
            left=self.children[0].get_ast(),
            op=self.get_operator(self.children[1]),
            right=self.children[2].get_ast()
        )
```
...
def get_ast(self) -> Expression:
    ...
    # X -> Y
        ...
        return self.children[0].get_ast()
```
...
def get_ast(self) -> Expression:
    # ...
    # F -> ( E )
    elif len(self.children) == 3 and self.children[1].symbol == Symbol.E:
        return self.children[1].get_ast()
```
...
def get_ast(self) -> Expression:
    # ...
    # i
    elif len(self.children) == 0 and self.symbol == Symbol.i:
        return Number(value=self.token_value)

    raise InvalidParseTreeException("xP")
```
...
class Node:
    ...
    def evaluate_synthesized(self):
        for node in self.children:
            node.evaluate_synthesized()

        for rule in self.rules:
            rule.evaluate(self.symbol, self.children)
```
...
class Node:
    def __init__(self):
        self.symbol: Symbol = Symbol.E
        self.children: list[Node] = []
        self.synthetic_rules: list[Rule] = []
        self.inherited_rules: list[Rule] = []

    def evaluate_inherited(self, parent: "Node"):
        for rule in self.inherited_rules:
            rule.evaluate(self.symbol, parent, parent.children)

        for node in self.children:
            node.evaluate_inherited(self)

        for rule in self.synthetic_rules:
            rule.evaluate(self.symbol, self.children)
```
