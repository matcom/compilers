...
class SumExpr(Expression):
    ...
    def check_types(self) -> bool:
        if not self.left.check_types() or not self.right.check_types():
            return False

        if self.left.node_type != Type.Integer or self.right.node_type != Type.Integer:
            return False

        self.node_type = Type.Integer
        return True
```
...
class IType(ABC):
    ...
    @abstractmethod
    def attributes(self) -> list[IAttribute]:
        ...
    ...
    @abstractmethod
    def get_attribute(self, name: str) -> IAttribute:
        ...

    @abstractmethod
    def get_method(self, name: str) -> IMethod:
        ...

    @abstractmethod
    def define_attribute(self, name: str, attr_type: IType) -> bool:
        ...

    @abstractmethod
    def define_method(self, name: str, return_type: IType,
                      arguments: list[str], argument_types: list[IType]) -> bool:
        ...
...
class IMethod(ABC):
    ...
    @abstractmethod
    def arguments(self) -> list[IAttribute]:
        ...
```
...
class IContext(ABC):
    ...
    @abstractmethod
    def get_type_for(self, symbol: str) -> IType:
        ...
    ...
    @abstractmethod
    def define_symbol(self, symbol: str, sym_type: IType) -> bool:
        ...

    @abstractmethod
    def create_type(self, name: str) -> IType:
        ...
```
...
class AttrDef(Node):
    def __init__(self):
        self.name: str = ""
        self.attr_type: str = ""
        self.initialization: Expression = None
```
...
```python
class IVisitor(ABC, Generic[TNode]):
    def visit(self, node: TNode):
        ...
```
...
```python
class TypeCollectorVisitor(IVisitor[Program], IVisitor[ClassDef]):
    def __init__(self):
        self.context: IContext = None
...
class Program(Node):
    ...
    def check_semantics(self, logger: IErrorLogger):
        type_collector = TypeCollectorVisitor()
        type_collector.visit(self)

        type_builder = TypeBuilderVisitor()
        type_builder.context = type_collector.context
        type_builder.visit(self)

        type_checker = TypeCheckerVisitor()
        type_checker.context = type_builder.context
        type_checker.visit(self, logger)
```
