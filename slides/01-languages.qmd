...
**Pregunta:** ¿Son estas dos operaciones (enumerar y reconocer) equivalentes? ¿Si tengo una ya tengo la otra?
...
eligiendo convenientemente la producción que "tocaría" aplicar (¿cómo?).
...
¡El próximo paso será formalizar este algoritmo!

# Concluyendo

Hoy hemos dado un primer vistazo a una idea central en Compilación, que va a resonar en todo el curso:

- Para resolver el problema de *parsing*, hemos creado un lenguaje declarativo de alto nivel (*gramáticas*), con el cual es posible (aunque todavía no sabemos cómo) construir automáticamente una máquina (formalmente un *autómata*) de parsing descrito en un lenguaje imperativo de bajo nivel.



¡Esto es exactamente la noción básica de "compilar", pero aplicada al propio proceso de compilación!



Intuitivamente, la idea que va a salir de aquí (y que demostraremos durante el curso) es que *los lenguajes y las máquinas son equivalentes*.
